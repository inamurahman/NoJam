...
class _LocationTrackerState extends State<LocationTracker> {
  ...
  double _totalReduction = 0; // Store the total reduction
  ...
  void _startTrackingLocation() {
    Geolocator.getPositionStream().listen((Position updatedPosition) {
      if (_previousPosition != null) {
        ...
        double totalReduction = calculateTotalReduction(mileage, selectedFuelType);
  ...
  double calculateTotalReduction(mileage, fuelType) {
    double totalReduction = 0.21;
    _weeklyData.forEach((date, data) {
      double walkingDistance = data['walking'] ?? 0.0;
      double cyclingDistance = data['cycling'] ?? 0.0;
      double reduction = calculateCarbonFootprint(walkingDistance+cyclingDistance, mileage, fuelType);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          FuturisticCarbonFootprintCard(reductionAmount: _totalReduction),
          Expanded(
            child: Center(
              child: Column(
...
double footprintReduction(
  ...
  // Calculate the carbon footprint of the car based on fuel type
  double carCarbonFootprintPerUnitDistance =
      calculateCarbonFootprint(1.0, mileage, carFuelType);

  // Calculate the total carbon footprint of walking and cycling
  double walkingCyclingCarbonFootprint =
      calculateCarbonFootprint((walkingDistance + cyclingDistance), mileage, carFuelType);

  // Handle cases where values are Infinity or NaN
  if (totalDistance.isInfinite || totalDistance.isNaN) {
    // Handle the case where totalDistance is Infinity or NaN
    return 0.0; // You can use a default value or handle it as needed
  }
  /*
  if (walkingCyclingCarbonFootprint.isInfinite || walkingCyclingCarbonFootprint.isNaN) {
    // Handle the case where walkingCyclingCarbonFootprint is Infinity or NaN
    return 0.0; // You can use a default value or handle it as needed
  }
  */
  // Calculate the reduction by subtracting walking and cycling carbon footprint
  // from the total carbon footprint
